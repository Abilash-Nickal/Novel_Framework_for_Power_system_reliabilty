...
def load_system_data(gen_file, load_file):
    ...
    Gen = gen_data['Unit Capacity (MW)'].values.astype(float)
    MTTF = gen_data['MTTF (hours)'].values.astype(float)
    MTTR = gen_data['MTTR (hours)'].values.astype(float)

    # --------- Merit-Order Sort (Cheapest First) for general dispatch -------------
    cost_colum = 'Unit Cost (LKR/kWh)'
    gen_data = gen_data.sort_values(by=cost_colum).reset_index(drop=True) #
    UnitCost = gen_data[cost_colum].values.astype(float)



    # --------- Identify Generators Types -------------
    is_coal = (gen_data['TYPES'].str.upper().str.contains('COAL')).values # Coal is base load
    is_hydro = (gen_data['TYPES'].str.upper().str.strip() == 'HYDRO').values # Hydro is seasonal with cap
