...
# (there are 5 dry,wet,medium,v.dry, v.wet)
...
def load_data():
    ...
    # Assuming the first column contains the hourly load data
...
def worker_smcs(worker_id, years_to_run, data_tuple):
    ...
    while current_time < target_sim_hrs:
        ...
        for i in range(num_of_gen):
            ...
            if AVAILABLE >= current_load: break
        ...
        for i in range(num_of_gen):
            ...
            # For hydro generators, check the monthly capacity limit.
            if is_hydro[i]:
                from_hydro = min(Gen[i], hydro_cap - hydro_used)
        ...
        is_failure = AVAILABLE < current_load - 1e-4
