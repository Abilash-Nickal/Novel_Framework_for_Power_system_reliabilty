...
def run_smcs(Gen, MTTF, MTTR, is_hydro, is_coal, UnitCost, Load, month_lookup):
    ...
    # --- NEW TRACKERS FOR LOLF AND LOLD ---
    total_lol_events = 0
    was_in_lol = False
    ...
    while current_time < TOTAL_HOURS:
        ...
        if is_failing:
            ...
            # If the system wasn't failing in the previous step, this is a NEW event!
            if not was_in_lol:
                total_lol_events += 1

        # Update the state tracker for the next iteration
        ...
        # Progress every 100 years (reduced from 1000 for better visibility)
        if int(current_time) % (HOURS_PER_YEAR * 100) < dt and current_time > 0:
            print(
                f"Progress: Year {int(current_time // HOURS_PER_YEAR)}, Time {current_time:.2f} hours, LOLP so far: {total_lol_hours / current_time:.8f}")
    ...
    LOLF = total_lol_events / NUM_YEARS

    # Safe division to prevent errors if the grid never failed
    if total_lol_events > 0:
        LOLD = total_lol_hours / total_lol_events
    else:
        LOLD = 0.0
    ...
    print(f"LOLF (Events/Year)       : {LOLF:.4f}")  # <--- Output added here
    print(f"LOLD (Hours/Event)       : {LOLD:.4f}")  # <--- Output added here
