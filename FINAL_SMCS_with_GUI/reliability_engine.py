...
def load_system_data(gen_file, load_file):
    ...
    """Loads and pre-processes the system data."""
    generator_data = pd.read_csv(gen_file)


    #------------   Merit-Order Sort (Cheapest First) for general dispatch -------------
    cost_colomn = 'Unit Cost (LKR/kWh)'
    generator_data = generator_data.sort_values(by=cost_colomn).reset_index(drop=True)
    UnitCost = generator_data[cost_colomn].values.astype(float)

    Gen = generator_data['Unit Capacity (MW)'].values.astype(float)
    MTTF = generator_data['MTTF (hours)'].values.astype(float)
    MTTR = generator_data['MTTR (hours)'].values.astype(float)

    is_hydro = (generator_data['TYPES'].str.upper().str.strip() == 'HYDRO').values
...
def run_full_sequential_simulation(num_years, data, update_queue=None):
    """
    The complete SMCS logic loop.
    Communicates progress to a GUI via update_queue if provided.
    """
    num_gen = len(data["Gen"])
    state = np.zeros(num_gen, dtype=int)  # 0 = UP, 1 = DOWN # Initial state: all generators are UP
    time_to_event = -data["MTTF"] * np.log(np.random.rand(num_gen)) # Initial time to failure for each generator

    total_lol_hours = 0.0
    total_loee = 0.0
    total_system_cost = 0.0
    total_lol_events = 0
    was_in_lol = False  # For event counting #

    current_time = 0.0
    total_target_hours = num_years * HOURS_PER_YEAR
    last_reported_year = 0

    # Wall clock for simulation time tracking
    ...
    while current_time < total_target_hours:
        ...
        if is_failing:
            ...
            # Count discrete LOL event (start of failure)
            if not was_in_lol:
                total_lol_events += 1
