...
class AdvancedSMCSGui:
    ...
    def start_sim(self):
        ...
        # Reset visual UI dashboards back to 0 immediately
        self.vars["y"].set("0")
        self.vars["lole"].set("0.0000")
        self.vars["lolp"].set("0.000000")
        self.vars["loee"].set("0.00")
        self.vars["events"].set("0")
        self.vars["cost"].set("0.00M")
        self.vars["sim_time"].set("0.0s")
            ...
            for _ in range(100):
                ...
                if msg["type"] == "update":
                    ...
                    if self.completed_workers >= self.target_workers:
                        ...
                        final_sim_time = time.time() - self.start_time
                        ...
                        if ty > 0:
                            ...
                            t_loee = self.total_loee / ty
                            t_cost = self.total_cost / ty
                            t_lolf = self.total_events / ty
                            t_lold = self.total_lol_hours / self.total_events if self.total_events > 0 else 0.0
                        else:
                            t_lole = t_lolp = t_loee = t_cost = t_lolf = t_lold = 0.0
