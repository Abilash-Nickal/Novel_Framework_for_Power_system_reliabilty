...
def load_data():
    ...
    try:
        ...
        # Using specific columns as defined by your query structure
        df_gen = pd.read_csv(GEN_DATA_FILE, header=0)

        Gen_df = df_gen[['Capacity (MW)', 'MTTF (hours)', 'MTTR (hours)']]
        ...
        if 'CEB_GEN_MTTR_MTTF.csv' in str(e):
            print(f"NOTE: Creating placeholder '{GEN_DATA_FILE}'. Please edit it with your real data.")
            # Use data similar to your uploaded example
            pd.DataFrame({
                'Capacity (MW)': [100.0, 200.0, 300.0, 50.0],
                'MTTF (hours)': [4380.0, 2190.0, 1500.0, 3000.0],
                'MTTR (hours)': [44.24, 44.69, 70.0, 30.0],
            }).to_csv("CEB_GEN_MTTR_MTTF.csv", index=False)
            print("Please ensure your load data file is also present and correctly named.")
        return None, None, None, None
    except Exception as e:
        print(f"FATAL ERROR during data loading: {e}. Check column names and types.")
        return None, None, None, None
...
def run_sequential_mcs(Gen, MTTF, MTTR, Annual_Load_Profile):
    ...
    # Check if the inputs are valid (MTTF and MTTR must be > 0)
    if not (np.all(MTTF > 0) and np.all(MTTR > 0)):
        print("ERROR: MTTF and MTTR must be positive numbers. Check generator data.")
        return
    ...
    while current_time < total_study_hours:
        ...
        annual_hour = int(current_time) % HOURS_PER_YEAR  # Use int() for index safety
        ...
        # We only consider positive times for the minimum event duration
        positive_times = time_to_next_event[time_to_next_event > 1e-6]  # Avoid near-zero issues
        min_time_step = np.min(positive_times) if positive_times.size > 0 else 1.0

        # Determine the actual time step to advance (cap at 1 hour for load fidelity)
        time_step = min(1.0, min_time_step)

        # Log progress periodically
        if int(current_time) % (HOURS_PER_YEAR) == 0 and current_time > 0:
            print(
                f"Progress: Year {int(current_time // HOURS_PER_YEAR)} completed. Total LOL Hours: {total_LOL_hours:,}")
        ...
        # c. Check for completed events (where time_to_next_event is near or below zero)
        units_changing_state = np.where(time_to_next_event <= 1e-6)[0]  # Check for near-zero

        for i in units_changing_state:
            # Recalculate time_to_next_event to ensure it's not negative (due to floating point math)
            time_to_next_event[i] = 0
...
if __name__ == "__main__":
    ...
    # 2. Run simulation
    if Gen is not None and Annual_Load_Profile is not None and Annual_Load_Profile.size > 0:
