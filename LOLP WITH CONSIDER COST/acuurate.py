...
def load_data():
    generator = pd.read_csv(GEN_DATA_FILE)
    ...
    unit_cost = 'Unit Cost (LKR/kWh)'
    if unit_cost not in generator.columns:
        # Fallback for testing if column name is different
        unit_cost = generator.columns[generator.columns.str.contains('Cost', case=False)][0]

    generator = generator.sort_values(by=unit_cost).reset_index(drop=True)

    Gen = generator['Unit Capacity (MW)'].values.astype(float)
    MTTF = generator['MTTF (hours)'].values.astype(float)
    MTTR = generator['MTTR (hours)'].values.astype(float)
    GenType = generator['TYPES'].values.astype(str)
    UnitCost = generator[unit_cost].values.astype(float)

    df_load = pd.read_csv(LOAD_DATA_FILE)
    Annual_Load = df_load.iloc[:, 0].values.astype(float)

    print(f"Data Loaded. Units sorted by cost. Cheapest: {generator['Name'].iloc[0] if 'Name' in generator else 'Unit 0'}")
...
def run_smcs(Gen, MTTF, MTTR, GenType, UnitCost, Load):
    num_of_gen = len(Gen)
    generator_state = np.zeros(num_of_gen, dtype=int)  # 0 = UP, 1 = DOWN
    time_to_event = -MTTF * np.log(np.random.rand(num_of_gen))
    ...
    while current_time < TOTAL_HOURS:
        ...
        # ---------------------------------------------
        up_generators = (generator_state == 0)
        ...
        for i in range(num_of_gen):
            if not up_generators[i]: continue  # Skip if unit is DOWN
        ...
        for i in events:
            generator_state[i] = 1 - generator_state[i]
            if generator_state[i] == 0:  # Repaired
