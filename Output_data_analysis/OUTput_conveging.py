...
def run_smcs(Gen, MTTF, MTTR, is_hydro, UnitCost, Load, month_lookup):
    ...
    ax.set_ylabel("LOLP (Convergence)")
    ax.set_title("SMCS Convergence of LOLP")
    ax.grid(True)
    ...
    while current_time < TOTAL_HOURS:
        ...
        # ---------------------------------------------
        # UPDATE LIVE CONVERGENCE GRAPH EVERY 10 YEARS
        # ---------------------------------------------
        current_year = int(current_time // HOURS_PER_YEAR)

        if current_year > 0 and current_year % 10 == 0 and hour_of_year == 0:
            current_LOLP = total_LOL_hours / current_time

            x_years.append(current_year)
            y_lolp.append(current_LOLP)

            line.set_xdata(x_years)
            line.set_ydata(y_lolp)
            ...
            print(f"Progress: Year {current_year}")
