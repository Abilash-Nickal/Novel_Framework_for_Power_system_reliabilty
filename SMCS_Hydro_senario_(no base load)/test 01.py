...
# =========================================================
NUM_YEARS = 100  # Set to 30,000 for final research results
HOURS_PER_YEAR = 8760
...
def load_data():
    ...
    # Boolean mask for faster inner-loop performance
    ...
    # Exact Month Lookup (Standard 8760-hour non-leap year)
...
def run_smcs(Gen, MTTF, MTTR, is_hydro, UnitCost, Load, month_lookup):
    ...
    # Initial time to failure sampling
    time_to_event = -MTTF * np.log(np.random.rand(num_of_generators))

    total_LOL_hours = 0.0
    total_LOEE = 0.0  # MWh (Energy Not Served)
    ...
    while current_time < TOTAL_HOURS:
        ...
        for i in range(num_of_generators):
            ...
            if is_hydro[i]:
                # Apply plant health and seasonal water limits
        ...
        if dispatched_power < current_load - 1e-4:
            ...
            total_LOL_hours += dt
            total_LOEE += deficit * dt  # MWh (Energy unserved in this step)
